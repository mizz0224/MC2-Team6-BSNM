//ToDO :  홈뷰로이동, 피드백 삭제, 이미지뷰의 selectedArr 배열 수정, 뷰 디테일 수정
...
struct DetailView: View {
    ...
    var selectedArrFrame : [Bool] = [Bool](repeating: false, count: 5)//ImageView의 selectedArr를 ImageView에서 만들지 못해 여기서 만들고 넘겨줌
    var body: some View {
        VStack(spacing:0) {
            //Spacer().frame(height:48)//상단 공백
            TopMenuView(idToDelete: getFeedback.id)//뒤로가기, 삭제 버튼 있는 뷰
            
            ImageView(getFeedback : getFeedback,selectedArr: selectedArrFrame)//이미지 뷰, 하단 설명뷰는 이미지 뷰에서 구현
